...
class AutoTagger:
    ...
    async def suggest_tags(self, content: str, title: str = "", existing_tags: Optional[List[str]] = None) -> List[TagSuggestion]:
        ...
        suggestions = [s for s in suggestions if s.tag not in existing_tags and s.confidence >= self.min_confidence]
    ...
    def _suggest_from_frequency(self, content: str) -> List[TagSuggestion]:
        ...
        return [TagSuggestion(kw, min(freq/10.0, 1.0)*0.6, f"Appears {freq}x", 'frequency') for kw, freq in keywords]
    ...
    async def _suggest_from_semantics(self, content: str) -> List[TagSuggestion]:
        ...
        return [TagSuggestion(kw, sim*0.9, f"Similar: {sim:.2f}", 'semantic') for kw, sim in keywords]
    ...
    def _suggest_from_structure(self, content: str, title: str) -> List[TagSuggestion]:
        ...
        suggestions = [TagSuggestion(content_type, 0.8, f"Type: {content_type}", 'structure')]
        if structure['code_block_count'] > 0:
            suggestions.append(TagSuggestion('code', 0.7, f"{structure['code_block_count']} blocks", 'structure'))
        return suggestions
    
    async def _suggest_from_related_notes(self, content: str) -> List[TagSuggestion]:
        try:
            similar = await self.kernel.retrieve_nodes_async(query=content[:500], top_k=5)
            tag_counts = {}
            for node in similar:
            ...
            total = len(similar)
            return [TagSuggestion(tag, (cnt/total)*0.7, f"{cnt}/{total} similar", 'existing') for tag, cnt in tag_counts.items()]
        except:
            return []
    ...
    def _merge_suggestions(self, suggestions: List[TagSuggestion]) -> List[TagSuggestion]:
        ...
        for tag, slist in tag_map.items():
            ...
            merged.append(TagSuggestion(tag, avg_conf, best.reason, ','.join(set(s.source for s in slist))))
        return merged
    ...
    def get_tag_stats(self) -> Dict[str, Any]:
        if not self._tag_history:
            return {'total_suggestions': 0, 'accepted_tags': [], 'rejected_tags': [], 'acceptance_rate': 0.0}
