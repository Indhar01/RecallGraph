...
class GapDetector:
    ...
    async def _detect_missing_topics(self) -> List[KnowledgeGap]:
        ...
        for keyword, freq in all_keywords.most_common(50):
            if keyword not in documented_topics and freq >= 3:
                ...
                found_in_title = any(
                    keyword in getattr(n, 'title', '').lower() 
                    for n in all_nodes
                )
    ...
    def _detect_isolated_notes(self) -> List[KnowledgeGap]:
        ...
        for node in all_nodes:
            ...
            # Count connections
            try:
                neighbors = self.kernel.graph.neighbors(node_id, depth=1, include_backlinks=True)
            ...
            if connection_count <= 1:
                ...
                content = getattr(node, 'content', '')
                keywords = self.analyzer.extract_keywords(content, max_keywords=10, min_frequency=1)
                ...
                for other_node in all_nodes:
                    ...
                    other_title = getattr(other_node, 'title', getattr(other_node, 'id', 'Unknown'))
    ...
    async def cluster_topics(self) -> List[TopicCluster]:
        ...
        for node in all_nodes:
            ...
            keywords = set(tags)
            extracted = self.analyzer.extract_keywords(content, max_keywords=10, min_frequency=1)
        ...
        dominant_keywords = [kw for kw, freq in all_keywords.most_common(20) if freq >= self.min_cluster_size]
            ...
            if len(cluster_nodes) >= self.min_cluster_size:
                ...
                if total_possible > 0:
                    for i, node_id in enumerate(cluster_nodes):
                        try:
                            neighbors_list = self.kernel.graph.neighbors(node_id, depth=1, include_backlinks=True)
                            neighbor_ids = set(n.id for n in neighbors_list)
                            actual_connections += len(neighbor_ids & set(cluster_nodes[i+1:]))
                        except:
    ...
    async def suggest_learning_paths(self, topic: str) -> List[LearningPath]:
        ...
        for node in all_nodes:
            ...
            if (topic_lower in title.lower() or
                topic_lower in content.lower() or
                any(topic_lower in tag.lower() for tag in tags)):
    ...
    def record_gap_feedback(self, gap_title: str, addressed: bool):
        ...
        self._gap_history[gap_title] = self._gap_history.get(gap_title, 0) + (1 if addressed else -1)
    ...
    def get_gap_stats(self) -> Dict[str, Any]:
        ...
        return {
            'total_gaps': total,
            'addressed_gaps': sorted(addressed.items(), key=lambda x: x[1], reverse=True),
