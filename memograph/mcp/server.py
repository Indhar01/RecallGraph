...
class MemoGraphMCPServer:
    ...
    def __init__(
        ...
        if llm_provider:
            logger.info(f"Initialized MemoGraph MCP server with vault: {self.vault_path}, provider: {llm_provider}")
        else:
            logger.info(f"Initialized MemoGraph MCP server with vault: {self.vault_path} (no LLM provider - client will handle answers)")
    ...
    async def query_with_context(
        ...
        """
        try:
            from ..core.assistant import retrieve_cited_context, run_answer, build_answer_prompt
