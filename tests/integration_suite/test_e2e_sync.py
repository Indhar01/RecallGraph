...
class TestMultiFileSync:
    ...
    @pytest.mark.asyncio
    async def test_selective_file_sync(
        self, sync_engine, integration_vault
    ):
...
class TestRealWorldPatterns:
    ...
    async def test_archive_workflow(self, sync_engine, integration_vault):
        ...
        assert "archive" in archived_nodes[0].frontmatter.get("meta", {}).get("obsidian_path", "")
...
class TestDataIntegrity:
    ...
    @pytest.mark.asyncio
    async def test_unicode_content_preservation(
        self, sync_engine, integration_vault
    ):
...
class TestSyncConsistency:
    ...
    @pytest.mark.asyncio
    async def test_sync_after_manual_changes(
        self, sync_engine, integration_vault
    ):
...
class TestConcurrentOperations:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_file_creation(
        self, sync_engine, integration_vault
    ):
