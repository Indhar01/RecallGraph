...
from memograph.integrations.obsidian.performance_metrics import PerformanceTracker, get_tracker, reset_tracker
...
class TestSQLitePerformance:
    ...
    def test_sqlite_vs_json_write_performance(self, temp_vaults):
        ...
        start = time.time()
        for i in range(1000):
            sqlite_state.update_file_hash(f"file_{i}.md", f"hash_{i}", 1024, time.time())
    ...
    def test_sqlite_statistics(self, sync_state):
        ...
        # Add some data
        for i in range(100):
            sync_state.update_file_hash(f"file_{i}.md", f"hash_{i}", 1024 * i, time.time())
...
class TestWikilinkResolution:
    ...
    def test_wikilink_resolution_with_index(self, parser, sample_notes):
        ...
        print(f"100 resolutions: {resolution_duration:.3f}s ({resolution_duration*10:.1f}ms each)")
